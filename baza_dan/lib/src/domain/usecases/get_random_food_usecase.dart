...
class GetRandomFoodUseCase implements FutureUseCase<dynamic, void>{
  final FoodRepository _foodRepository;
  const GetRandomFoodUseCase(this._foodRepository);

  @override
  Future call({void params}) async{
    var info = await _foodRepository.getRandomFood();
    if(info != null && info.toString().isNotEmpty && info!= '""'){
      ...
      for(int i=0; i<len; i++){
        ...
        String food_img = getFoodPhoto(int.parse(data1['food_id']));
        data1.addAll({
          'food_img': food_img,
        });
        foodList.add(data1);
